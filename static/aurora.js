...
float auroraGlow(float t, vec2 shift, vec2 resolution, float noiseFreq, float noiseAmp, float scale, float octaveDecay, float bandHeight, float bandSpread) {
...
void main() {
    ...
    vec3 col = vec3(0.0);
    // User requested colors: #f7f7f7 and #e100ff
    col += 0.99 * auroraGlow(t, shift, uResolution.xy, uNoiseFreq, uNoiseAmp, uScale, uOctaveDecay, uBandHeight, uBandSpread) * cosineGradient(uv.x + uTime * uSpeed * 0.2 * uColorSpeed, vec3(0.5), vec3(0.5), vec3(1.0), vec3(0.3, 0.20, 0.20)) * uColor1;
    col += 0.99 * auroraGlow(t + uLayerOffset, shift, uResolution.xy, uNoiseFreq, uNoiseAmp, uScale, uOctaveDecay, uBandHeight, uBandSpread) * cosineGradient(uv.x + uTime * uSpeed * 0.1 * uColorSpeed, vec3(0.5), vec3(0.5), vec3(2.0, 1.0, 0.0), vec3(0.5, 0.20, 0.25)) * uColor2;
...
export function initAurora(containerId) {
